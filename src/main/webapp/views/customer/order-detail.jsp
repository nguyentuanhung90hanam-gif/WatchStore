...
<div class="order-detail-head"><div><p class="eyebrow dark">MÃ ĐƠN #${order.code}</p><h1>Chi tiết đơn hàng</h1><span>Đặt lúc ${order.createdAt}</span></div><span class="status-badge ${order.statusCssClass}">${order.statusLabel}</span></div>
...
<aside class="detail-card payment-summary"><h2>Thanh toán</h2><p><span>Tổng đơn hàng</span><b><fmt:formatNumber value="${order.total}" pattern="#,##0"/>₫</b></p><p><span>Trạng thái thanh toán</span><b>${order.status}</b></p>

...
<c:forEach items="${featuredProducts}" var="cardProduct"><c:if test="${cardProduct.id == param.productId}">
    <article class="product-card pro-product-card">
        <div class="product-media">
            ...
            <c:set var="cardImg" value="${not empty cardProduct.imageUrl ? cardProduct.imageUrl : cardProduct.image}" />
            <a class="product-image-link" href="${cp}/page/product?id=${cardProduct.id}">
                <c:choose>
                    <c:when test="${fn:startsWith(cardImg, 'http://') || fn:startsWith(cardImg, 'https://')}">
                        <img src="${cardImg}" alt="${cardProduct.name}" loading="lazy"
                             onerror="this.onerror=null;this.src='${cp}/assets/images/watch-1.png';">
                    </c:when>
                    <c:when test="${fn:startsWith(cardImg, '/')}">
                        <img src="${cp}${cardImg}" alt="${cardProduct.name}" loading="lazy"
                             onerror="this.onerror=null;this.src='${cp}/assets/images/watch-1.png';">
                    </c:when>
                    <c:otherwise>
                        <img src="${cp}/assets/images/${cardImg}" alt="${cardProduct.name}" loading="lazy"
                             onerror="this.onerror=null;this.src='${cp}/assets/images/watch-1.png';">
                    </c:otherwise>
                </c:choose>
            </a>

...
public class ProductRepositoryImpl implements ProductRepository {
    ...
    private Product mapRow(ResultSet rs) throws SQLException {
        ...
        if (rs.getObject("CreatedBy") != null) p.setCreatedBy(rs.getInt("CreatedBy"));
        ...
        try { p.setSku(rs.getString("SKU")); } catch (SQLException ignored) {}
        try { p.setStock(rs.getInt("TotalStock")); } catch (SQLException ignored) {}

        try {
            String img = rs.getString("PrimaryImageUrl");
            p.setImageUrl(img != null && !img.isBlank() ? img : "watch-1.png");
        } catch (SQLException ignored) {}
        ...
        return """
            SELECT p.*,
                   ...
                   (SELECT TOP 1 ImageUrl
                    ...
                    ORDER BY pi.IsPrimary DESC, pi.DisplayOrder ASC) AS PrimaryImageUrl
            FROM Products p
    ...
    private void upsertPrimaryImage(Connection con, int productId, String imageUrl, String productName)
            throws SQLException {
        ...
        String updateSql = """
            ...
            SET ImageUrl = ?, AltText = ?, IsPrimary = 1, DisplayOrder = 1
            WHERE ProductID = ? AND IsPrimary = 1
            """;

        try (PreparedStatement ps = con.prepareStatement(updateSql)) {
            ps.setString(1, imageUrl.trim());
            ps.setString(2, productName != null && !productName.isBlank()
                    ? "Ảnh chính " + productName
                    : "Ảnh chính sản phẩm");
        ...
        try (PreparedStatement ps = con.prepareStatement(insertSql)) {
            ...
            ps.setString(3, productName != null && !productName.isBlank()
                    ? "Ảnh chính " + productName
                    : "Ảnh chính sản phẩm");
            ps.executeUpdate();
        }
            ...
            try (PreparedStatement ps = con.prepareStatement(sqlProduct, Statement.RETURN_GENERATED_KEYS)) {
                ...
                try (ResultSet rs = ps.getGeneratedKeys()) {
                    ...
                    String sku = product.getSku() != null && !product.getSku().isBlank()
                            ? product.getSku() : product.getProductCode() + "-STD";

                    String sqlVariant = """
                        INSERT INTO ProductVariants
                            (ProductID, SKU, VariantName, CostPrice, SalePrice, CompareAtPrice, Status)
                        VALUES (?, ?, ?, ?, ?, ?, 'ACTIVE')
                        """;
                    ...
                    try (PreparedStatement psVar = con.prepareStatement(sqlVariant)) {
                        ...
                        psVar.setBigDecimal(4, product.getPrice() != null
                    ...
                    upsertPrimaryImage(con, productId, product.getImageUrl(), product.getProductName());
                }
    ...
    public boolean update(Product product) {
        ...
        try (Connection con = getConnection()) {
            ...
            try (PreparedStatement ps = con.prepareStatement(sqlProduct)) {
                ...
                upsertPrimaryImage(con, product.getProductId(), product.getImageUrl(), product.getProductName());
    ...
    public boolean existsByCode(String code, Integer excludeId) {
        ...
        try (Connection con = getConnection();
            ...
            if (excludeId != null && excludeId > 0) ps.setInt(2, excludeId);

...
public class CartController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if ("/remove".equals(path)) {
            ...
            cart.put(id, Math.max(1, parse(req.getParameter("quantity"), 1)));
        } else {
            cart.merge(
                    id,
                    Math.max(1, parse(req.getParameter("quantity"), 1)),
                    Integer::sum
            );
        }
    ...
    private void prepare(HttpServletRequest req) {
        ...
        req.setAttribute(
                "discount",
                subtotal.compareTo(new BigDecimal("10000000")) >= 0
        ...
        req.setAttribute(
                "shipping",
                subtotal.signum() == 0
                        || subtotal.compareTo(new BigDecimal("1000000")) >= 0
                        ? BigDecimal.ZERO

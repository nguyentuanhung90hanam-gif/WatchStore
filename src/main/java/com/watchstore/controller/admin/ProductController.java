...
public class ProductController extends HttpServlet {
                    ...
                    try {
                        ...
                        if (productRepository.isProductInUse(id)) {
                            req.getSession().setAttribute("errorMessage",
                                    "Không thể xóa sản phẩm này vì đã có dữ liệu liên quan (đơn hàng, giỏ hàng, đánh giá, kho hàng...).");
                        } else {
    ...
    private void handleSave(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (error != null) {
            Product draft = buildProduct(0, code, name, slug, brandId, categoryId, price, sku,
                    movementType, gender, status, parseBoolean(isFeaturedStr), imageUrl, description);
            showFormWithError(req, resp, draft, error, "Thêm sản phẩm");
            return;
        }

        Product product = buildProduct(0, code, name, slug, brandId, categoryId, price, sku,
                movementType, gender, status, parseBoolean(isFeaturedStr), imageUrl, description);

        if (!productRepository.insert(product)) {
            showFormWithError(req, resp, product,
                    "Không thể thêm sản phẩm. Vui lòng kiểm tra lại dữ liệu.", "Thêm sản phẩm");
            return;
        }
    ...
    private void handleUpdate(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (error != null) {
            Product draft = buildProduct(id, code, name, slug, brandId, categoryId, price, sku,
                    movementType, gender, status, parseBoolean(isFeaturedStr), imageUrl, description);
            showFormWithError(req, resp, draft, error, "Sửa sản phẩm");
            return;
        }

        Product product = buildProduct(id, code, name, slug, brandId, categoryId, price, sku,
                movementType, gender, status, parseBoolean(isFeaturedStr), imageUrl, description);

        if (!productRepository.update(product)) {
            showFormWithError(req, resp, product,
                    "Không thể cập nhật sản phẩm. Vui lòng kiểm tra lại dữ liệu.", "Sửa sản phẩm");
            return;
        }
    ...
    private void showFormWithError(HttpServletRequest req, HttpServletResponse resp,
                                   Product draft, String error, String pageTitle)
            throws ServletException, IOException {
    ...
    private String validateProduct(String code, String name, String slug, String sku,
                                   int brandId, int categoryId, BigDecimal price,
                                   String movementType, String gender, String status) {
        ...
        if (!movementType.isEmpty()
                && !"AUTOMATIC".equals(movementType)
                && !"QUARTZ".equals(movementType)
                && !"SOLAR".equals(movementType)
                && !"SMART".equals(movementType)
                && !"MECHANICAL".equals(movementType)) {
        ...
        if (!gender.isEmpty()
                && !"MEN".equals(gender)
                && !"WOMEN".equals(gender)
                && !"UNISEX".equals(gender)
                && !"COUPLE".equals(gender)) {
    ...
    private Product buildProduct(int id, String code, String name, String slug,
                                 int brandId, int categoryId, BigDecimal price, String sku,
                                 String movementType, String gender, String status,
                                 boolean isFeatured, String imageUrl, String description) {
